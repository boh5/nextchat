...
export async function getFriendsList(cursor?: string): Promise<ListResponse> {
  ...
  const friendsList = await db
    ...
    .innerJoin(
      users,
      or(
        and(eq(friends.friendId, users.id), eq(friends.userId, session.user.id)),
...
export async function sendMessage(data: {
  ...
  if (data.type === 'private') {
    ...
    const isFriend = await db
      ...
      .where(
        and(
          eq(friends.status, 'accepted'),
          or(
            and(eq(friends.userId, userId), eq(friends.friendId, data.receiverId)),
            and(eq(friends.userId, data.receiverId), eq(friends.friendId, userId))
          )
        )
      )
      .limit(1)
    ...
    const isMember = await db
      .select()
      .from(groupMembers)
      .where(and(eq(groupMembers.groupId, data.receiverId), eq(groupMembers.userId, userId)))
      .limit(1)
...
export async function markAsRead(targetId: string, type: 'private' | 'group') {
  ...
  if (type === 'private') {
    ...
    await db
      ...
      .where(and(eq(groupMembers.groupId, targetId), eq(groupMembers.userId, userId)))
  }
